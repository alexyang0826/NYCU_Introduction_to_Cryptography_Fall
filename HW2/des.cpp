...
string encrypt(string pt, vector<string> rkb, vector<string> rk,int a)
{
	if (a == 0) {	
		// ascii to bin
		pt = ascii2bin(pt);
...
int main()
{
	...
	cout << "\nEncryption:\n";
	string cipher = encrypt(pt, rkb, rk, 0);
	cout << "\nCipher Text: " << cipher << endl;

	cout << "\nDecryption:\n";
	ifstream fin("DES-Key-Ciphertext.txt", ios::in);
	ofstream fout("des-out.txt", ios::out|ios::trunc);
	while (!fin.eof()) {
		fin >> key >> ct;
		key = ascii2bin(key);
		// getting 56 bit key from 64 bit using the parity bits
		...
		string text = encrypt(ct, rkb, rk, 1);
		cout << "\nPlain Text: " << hex2ascii(text) << endl;
		fout << hex2ascii(text) << endl;
	}
